<script>
import {addCategory, addFood} from "@/api/services";
...
export default {
    ...
    if (this.$route.query.restaurant_id) {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            this.$router.push('/dataManage/merchant');
            done();
          } else {
            this.$message({
              type: 'info',
  ...
  methods: {
    async initData() {
      try {
        const result = await this.$store.dispatch('dataManage/getRestaurantCategory', {restaurant_id: this.restaurant_id});
        this.categoryForm.categoryList = result;
      } catch (e) {
        ...
        <el-form :model="foodForm" :rules="foodRules" ref="foodForm" label-width="110px" class="form food_form">
          ...
          <el-form-item label="上传食品图片">
            <el-upload
              ...
              <img v-if="foodForm.image_path" :src="baseImgPath + foodForm.image_path" class="avatar" alt="">
